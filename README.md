...
We think that we will add RSA encryption method to hex obfuscator in the future, so new desired encryption methods can be added here: https://github.com/Celuk/ERIC/blob/main/clangport-and-elf2encryptedhex/src/elf2encryptedhex/elf2encryptedhex.cpp#L3397
   
Also, new desired encryption methods can be automatically generated by user in IDE in the future, here is the considered method:

```bash
Wrote Encryption Algorithm In C++ In encIDE --> Generate Encryptor cpp File with CMake and Add Encryption Option --> Build ERIC Again
```
